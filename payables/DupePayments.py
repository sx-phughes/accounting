...
def get_n_months_payble_paths(
    ...
    months_to_search = [
        payables_dt - timedelta(30 * i) for i in range(1, n_months + 1)
    ] + [payables_dt]
    ...
    for dt in months_to_search:
        ...
        folders = list(
            filter(
                ...
                and x != payables_dt.strftime("%Y-%m-%d"),
                dir_contents,
            )
        )
    ...
    for f_grp in payables_folders:
        ...
        for dt in f_grp[1]:
            payables_file_stems.append(
                f_grp[0] + "/" + dt + "/" + dt + " Payables.xlsm"
            )
...
def find_dupe_invoices(
    ...
    try:
        ...
        curr_payables_df = pd.read_excel(
            payables_root + "/" + curr_payables_stem.replace(".xlsm", ".xlsx"),
            "Invoices",
        )
    curr_payables_df["Concat"] = (
        curr_payables_df["Vendor"]
        + ","
        + curr_payables_df["Invoice #"].astype(str)
    )
...
def search_for_dupe_payments(
    date: str, months_back: int, save_dir: str
) -> pd.DataFrame:
...
def main():
    date = input(
        "Input date of payables batch to check for dupes (yyyy-mm-dd):\n>\t"
    )
    n_months = input("Input number of months to check back:\n>\t")
    save_path = input(
        "Input save path for dupe invoice check (optional, defaults to pwd):\n>\t"
    )
