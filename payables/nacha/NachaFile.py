...
class Batch:
    def __init__(
        ...
        orig_dfi_id,
        batch_number,
        trx_entries: list[TransactionEntry],
    ):
    ...
    def __str__(self):
        str_list = (
            [self.header_line.__str__()]
            + [i.__str__() for i in self.trx_entries]
            + [self.footer_line.__str__()]
        )
        batch_str = "\n".join(str_list)
        return batch_str
...
class NachaFile:
    ...
    def __str__(self):
        str_list = (
            [self.file_header.__str__()]
            + [i.__str__() for i in self.batches]
            + [self.file_footer.__str__()]
        )
        text = "\n".join(str_list)
