...
from pages import getDiscoveryDocument
...
def signInWithIntuit(request):
    url = getDiscoveryDocument.auth_endpoint
    scope = " ".join(
        settings.OPENID_SCOPES
    )  # Scopes are required to be sent delimited by a space
    params = {
        "scope": scope,
        "redirect_uri": settings.REDIRECT_URI,
        "response_type": "code",
...
def authCodeHandler(request: WSGIRequest):
    ...
    #     return HttpResponse(
    #         "JWT Validation failed. Please try signing in again"
    #     )
    # else:
    #     return redirect("connected")
    # else:
    if bearer.idToken is None:
        return redirect("connected")
...
def disconnect(request):
    ...
    if access_token is not None:
        ...
        return HttpResponse(
            "No accessToken or refreshToken found, Please connect again"
        )
...
def apiCall(request):
    ...
    if status_code >= 400:
        ...
        bearer = getBearerTokenFromRefreshToken(refresh_token)
        updateSession(
            request, bearer.accessToken, bearer.refreshToken, realmId
        )
