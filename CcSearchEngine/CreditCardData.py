...
class CreditCardData(ABC):
    def __init__(self):
        ...
        #     self.data["Transaction Date"], format="mixed", dayfirst=False
        # )
    ...
    def save_data(self, data: pd.DataFrame):
        ...
        full_path = self.save_path + "/search_results_{now}.xlsx".format(
            now=now
        )
    ...
    def search_by_description(self, search_term: str):
        results = self.data[
            self.data["Description"].str.contains(
                search_term, na=False, flags=re.IGNORECASE
            )
        ]
    ...
    def search_by_memo(self, memo: str):
        results = self.data[
            self.data["Memo"].str.contains(memo, na=False, flags=re.IGNORECASE)
        ]
        results = results.copy()
        results = results.reset_index(drop=True)
    ...
    def search_by_date(self, min_date, max_date):
        ...
        if min_date and max_date:
            results = self.data[
                (self.data["Transaction Date"].dt.date > min_date)
                & (self.data["Transaction Date"].dt.date < max_date)
            ]
    ...
    def get_data(self):
        ...
        for i in years:
            for j in months:
                ...
                f_pattern = rf"{yearmonth} - Chase CC Reconciliation"
                    ...
                    for file in files:
                        ...
                        if match:
                            ...
                            if yearmonth in workbook_names["Period"]:
                                ...
                                curr_file_mod_t = os.path.getmtime(
                                    curr_full_path
                                )

                                old_index = workbook_names["Period"].index(
                                    yearmonth
                                )
                                old_file = (
                                    workbook_names["Path"][old_index]
                                    + "/"
                                    + workbook_names["Filename"][old_index]
                                )
                                old_file_mod_t = os.path.getmtime(old_file)

                                if curr_file_mod_t > old_file_mod_t:
                                    workbook_names["Filename"][
                                        old_index
                                    ] = match.string
        ...
        for i in range(len(workbook_names["Period"])):
            full_path = (
                workbook_names["Path"][i] + "/" + workbook_names["Filename"][i]
            )
            if ".csv" in full_path:
                continue
            workbook = pd.ExcelFile(full_path, engine="openpyxl")
            ...
            for j in sheets:
                ...
                if sheet_match:
                    try:
                        ...
                        workbook_names["Detail Sheet"].append(
                            sheet_match.string
                        )
