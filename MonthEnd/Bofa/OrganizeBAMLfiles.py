...
class BAMLFileMover:
    ...
    def getFileNames(self):
        ...
        for i in self.folders:
            for j in patterns:
                ...
                full_file_path = (
                    BAMLFileMover.search_path + f"/{i}/{formatted_file_name}"
                )
    ...
    def process_bookkeeping(self):
        ...
        for file in files_with_path:
            df = pd.read_csv(
                file, names=comb_headers.div_headers, low_memory=False
            )
        ...
        gl_detail.to_csv(
            self.bofa_folder + "/GL_Detail_Table.csv", index=False
        )
        div_sum_df.to_csv(
            self.bofa_folder + "/Dividends_Table.csv", index=False
        )
        all_div_df.to_csv(
            self.bofa_folder
    ...
    def get_gl_details(self, i_df, gl_det_df):
        ...
        month-end workflow"""
        gl_det_i = i_df.loc[
            i_df["Type of Entry"].isin(
                ["INTE", "FSD", "WCK", "FPE", "GJE", "JE", "INTI"]
            )
        ].copy()
    ...
    def get_div_detail(self, i_df, div_detail_df):
        ...
        table"""
        div_df = i_df[
            (i_df["Origin Code"] != "DIVP") & (i_df["Origin Code"] == "DV")
        ].copy()
