...
def get_combined_data() -> pd.DataFrame:
    merged = merge_ledger_map_to_data(get_pm_data(), get_cm_data())
    return merged
...
def merge_ledger_map_to_data(
    left_data: pd.DataFrame, right_data: pd.DataFrame
) -> pd.DataFrame:
    data = left_data.merge(
        right=right_data, how="left", left_on="ABN Map", right_on="Concat"
    )
...
def rename_for_final(data: pd.DataFrame) -> None:
    renamer = {
        ...
        Base.cm_moyr: datetime.strptime(
            Base.cm_moyr + "01", "%Y%m%d"
        ).strftime("%b %Y\t"),
        Base.pm_moyr: datetime.strptime(
            Base.pm_moyr + "01", "%Y%m%d"
        ).strftime("%b %Y\t"),
    }
...
def finalize_data(data: pd.DataFrame) -> pd.DataFrame:
    merged_accounts = data.merge(
        Base.account_mapping, "left", left_on="AccountID", right_on="ACCOUNT"
    )
