...
def get_account_name(page: pypdf.PageObject) -> str:
    ...
    account_match = re.search(
        r"(ACCOUNT[\s]*:\s)([\d]*\s)([A-Z\d]*)", account_line
    )
...
def get_data_table(f: str, page: pypdf.PageObject) -> pd.DataFrame:
    ...
    account_name = account_match.group(3)
    print(
        "Converting account {account_name}".format(account_name=account_name)
    )
    ...
    for line in table_by_line:
        ...
        if search:
            ...
            for i in range(len(table_dict.keys())):
                table_dict[list(table_dict.keys())[i]].append(
                    search.group(relevant_groups[i])
                )
        else:
            continue
