from oldFileGrabber import AbnFileGrabber
import shutil, os, re
import pandas as pd
...
def rename_files():
    ...
    strategy_mapping = pd.read_csv(
        "C:/gdrive/Shared drives/accounting/patrick_data_files/abn_month_end/ABN_account_mapping.csv"
    )
    year = 2024
    ...
    renamer = {
        account_no[0] + "_" + account_no[1]: file_index
        for file_index, account_no in account_mapping.items()
    }
    # print(renamer)
    ...
    for file in os.listdir():
        ...
        if search:
            ...
            account_strategy_i = strategy_mapping.loc[
                strategy_mapping["ACCOUNT"] == account_no, "Strategy"
            ].index[0]
            account_strategy = strategy_mapping.iloc[account_strategy_i, 1]

            new_name = (
                f"{date}_{account_strategy}_{account_no}_Monthly Statement.pdf"
            )
...
def grab_files(year: int):
    ...
    copy_dir = "C:/Users/Patrick/Downloads"
    ...
    for i in account_mapping.keys():
        print(
            f"Account {i} has file index {account_mapping[i][0]}_{account_mapping[i][1]}"
        )
    ...
    for month in files_to_grab.keys():
        ...
        for account in files_to_grab[month]:
            ...
            stmt_path, stmt_f_name = grabber.get_abn_pdf_monthly_statement(
...
def new_grab_files(year: int, ptr_table: pd.DataFrame):
    """New method for grabbing files with more selectivity.

    year: year files needed from
    ptr_table: a DataFrame with columns Trade Date and Account to point the grabber at the correct statement.
    """
    copy_dir = "C:/Users/Patrick/Downloads"
    ...
    for i, row in ptr_table.iterrows():
        date = row["Trade Date"]
        account = row["Account"]
        grabber = AbnFileGrabber(date.year, date.month)
        stmt_path, stmt_f_name = grabber.get_abn_pdf_monthly_statement(
            account, account_mapping, date.day
        )
        ...
        if stmt_f_name:
            ...
            print(
                f"File for account {account} in month {date.month} not found"
            )
...
if __name__ == "__main__":
    ...
    abn_only_input = input_file.loc[input_file["Broker"] == "ABN"].copy()
    new_grab_files(2025, abn_only_input)
