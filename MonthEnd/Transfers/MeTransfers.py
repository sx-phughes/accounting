...
def run_baml_table(date: datetime, save_path="."):
    ...
    for i, row in df.iterrows():
        if row["Account No."] in baml_accounts:
            i_row = [row["Account No."], round(float(row["Equity"]), 2)]
...
def run_abn_tables(date: datetime, save_path="."):
    ...
    data_ref = {"695": eqt_data, "813": eqt_data, "Fut": fut_data}
    ...
    for acct in et_table["Account"]:
        et_tfr_table.add_data_row(
            acct,
            et_table.loc[et_table["Account"] == acct, "Transfer"].values[0],
            flip=False,
        )

    et_tfr_table.add_final_row()

    eightone_tfr_table["Firm"] = eightone_tfr_table["Firm"].apply(
        lambda x: zero_pad(x, 3)
    )

    for acct_group in table_dict.keys():
        table_dict[acct_group].to_csv(
            f"{save_path}/{date_str}{f_names[acct_group]}", index=False
        )

    et_tfr_table.to_csv(f"{save_path}/{date_str}{f_names['ET']}", index=False)


def run_ME_Transfers(
    year, month, day, save_path=os.environ["HOMEPATH"] + "\\Downloads"
):
